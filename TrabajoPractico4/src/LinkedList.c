...
/*
    ...
    if(this!= NULL && index>=0 && index<this->size){
        ...
        if(index==0 && this->size==2){
            pAuxNode=getNode(this,1);
            this->pFirstNode->pNextNode=pAuxNode->pNextNode;
            this->size--;
            free(pAuxNode);
            returnAux=0;
...
LinkedList* ll_map(LinkedList* this, void* (*pFunc)(void*)){
    ...
    LinkedList* AuxReturn=NULL;
    void* pAuxThis;
    void* pAux;
    int len;

    if(this!=NULL && pFunc!=NULL && this->size>0){
        len=ll_len(this);
        AuxReturn=ll_newLinkedList();
        if(AuxReturn!=NULL){
            for(int i=0; i<len;i++){
                pAuxThis=ll_get(this,i);
                if(pAuxThis!=NULL){
                    pAux=pFunc(pAuxThis);
                    ll_add(AuxReturn,pAux);
                }
            }
